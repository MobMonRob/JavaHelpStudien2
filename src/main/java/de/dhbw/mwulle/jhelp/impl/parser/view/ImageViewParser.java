...
public class ImageViewParser implements ViewParser {
    ...
    public void parse(ViewBuilder builder, Element element) {
        if (builder.getImage() != null) {
            throw new IllegalStateException(String.format("Image was already set to '%s' but got other image node with value %s", builder.getImage(), element.getNodeValue()));
        }

        builder.setImage(element.getNodeValue());
    }
}

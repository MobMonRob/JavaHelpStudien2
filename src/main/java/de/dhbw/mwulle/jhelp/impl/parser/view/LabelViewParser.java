...
public class LabelViewParser implements ViewParser {
    ...
    public void parse(ViewBuilder builder, Element element) {
        if (builder.getLabel() != null) {
            throw new IllegalStateException(String.format("Label was already set to '%s' but got other label node with value %s", builder.getLabel(), element.getTextContent()));
        }

        builder.setLabel(element.getTextContent());
    }
}
